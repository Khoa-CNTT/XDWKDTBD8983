...
if (isset($_SESSION['cart'])) {
    foreach ($_SESSION['cart'] as $ds) {
        $dongia = is_numeric($ds['dongia']) ? (float)$ds['dongia'] : 0;
...
if ($loi || ($tong + $ship <= 0)) {
    echo "<script>
            alert('Không thể thanh toán: Sản phẩm có đơn giá hoặc số lượng không hợp lệ!');
            window.location.href='listcart.php?error=invalid';
          </script>";
    exit();
}
